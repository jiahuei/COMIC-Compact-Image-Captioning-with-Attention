...
randomise = False
output_dir = '/home/jiahuei/Documents/supplemental/mscoco_bN256_beam_3'
...
def save_files(name, img, img_id, model_cap, base_cap):
    # Save image
    ...
    with open(pjoin(output_dir, 'captions_{}.txt'.format(name)), 'a') as f:
        f.write('{}\r\n{}'.format(fname, out_str))
    
    # Write captions in LATEX format
    out_str = '\t\\gph{{1.0}}{{images/supplemental/xxx/{}}}  &\n'.format(fname)
    out_str += '\t\\begin{tabular}{M{\\linewidth}}\n'
    out_str += '\t\t\\begin{modcap}\n'
    out_str += '\t\t\t{}\n'.format(model_cap)
    out_str += '\t\t\\end{modcap} \\\\\n'
    out_str += '\t\t\\begin{basecap}\n'
    out_str += '\t\t\t{}\n'.format(base_cap)
    out_str += '\t\t\\end{basecap} \\\\\n'
    out_str += '\t\\end{tabular} &\n\n'
    
    with open(pjoin(output_dir, 'captions_latex_{}.txt'.format(name)), 'a') as f:
        f.write(out_str)


for score in scores[4800:]:
    ...
    # Get key press
    key_input = raw_input('"z" if both wrong, "b" for baseline, "m" for model, or other key to skip.\n')
    if key_input == 'z':
        # Both wrong
        save_files('wrong', img, img_id, model_cap, base_cap)
    
    elif key_input == 'b':
        # Baseline correct
        save_files('baseline', img, img_id, model_cap, base_cap)
    
    elif key_input == 'm':
        # Model correct
        save_files('model', img, img_id, model_cap, base_cap)
